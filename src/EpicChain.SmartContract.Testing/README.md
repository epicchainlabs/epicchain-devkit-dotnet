...
This class provides precompiled artifacts for epicchain's native contracts, thereby simplifying and facilitating calls to native contracts.
...
Imagine that our project checks that our account has a balance of 123 EpicChain. It would be enough to redirect the calls to the EpicChain `balanceOf` method in the following way, so that it always returns 123.
...
// Get EpicChain Coverage (NULL)

Assert.IsNull(engine.GetCoverage(engine.Native.EpicChain));
...
Assert.AreEqual(3, engine.GetCoverage(engine.Native.EpicChain)?.CoveredInstructions);
Assert.AreEqual(3, engine.GetCoverage(engine.Native.EpicChain)?.HitsInstructions);
```
